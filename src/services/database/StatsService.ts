// src/services/database/StatsService.ts
import { DBPlayer, DBMatch, DBMatchPlayer, TABLES, TRUESKILL_BETA, TRUESKILL_TAU } from './models';
...
/**
 * Service for player statistics and rating calculations
 */
class StatsService {
  ...
  async recalculatePlayerStats(): Promise<void> {
    try {
      ...
      let allMatches = await matchService.getAllMatches();
      allMatches.sort((a, b) => {
      ...
      for (const match of allMatches) {
        const matchPlayers = await matchService.getMatchPlayers(match.id);
      ...
      for (const player of players) {
        ...
        await playerService.savePlayer(updatedPlayer);
      }
  ...
  async getPlayerStats(playerId: string): Promise<{
    ...
    const matchesPlayed = await matchService.getPlayerMatches(playerId);
  ...
  async getHeroStats(): Promise<any[]> {
    try {
      const allMatchPlayers = await matchService.getAllMatchPlayers();
