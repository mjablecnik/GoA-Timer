// src/services/firebase/FirebaseMatchMakingService.ts
import { TRUESKILL_BETA, TRUESKILL_TAU } from '../database/models';
import { rating } from 'openskill';
import NormalDistribution from 'normal-distribution';
import firebasePlayerService from './FirebasePlayerService';

/**
 * Service for matchmaking, team balancing, and win probability calculations with Firestore
 */
class FirebaseMatchMakingService {
  ...
  async generateBalancedTeams(playerIds: string[]): Promise<{ team1: string[], team2: string[] }> {
    const allPlayers = await firebasePlayerService.getAllPlayers();
    ...
    selectedPlayers.sort((a, b) => {
      const ratingA = firebasePlayerService.getDisplayRating(a);
      const ratingB = firebasePlayerService.getDisplayRating(b);
    ...
    selectedPlayers.forEach(player => {
      const team1Skill = team1.reduce((sum, p) => sum + firebasePlayerService.getDisplayRating(p), 0);
      const team2Skill = team2.reduce((sum, p) => sum + firebasePlayerService.getDisplayRating(p), 0);
...
export const firebaseMatchMakingService = new FirebaseMatchMakingService();
export default firebaseMatchMakingService;